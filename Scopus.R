...
searchByString <- function(string, content = "standard", myStart = 0, retCount = 25, retMax = Inf, mySort = "-coverDate", cursor = "*", outfile) {
	if (!content %in% c("complete", "standard")) {
		...
		theURL <- httr::GET("https://api.elsevier.com/content/search/scopus", query = list(apiKey = key, query = string, sort = mySort, httpAccept = "application/xml", view = "standard", count = retCount, start = myStart, cursor = cursor)) ## format the URL to be sent to the API
		...
		while (resultCount > retrievedCount && retrievedCount < retMax) { ## check if it's necessary to perform multiple requests to retrieve all of the results; if so, create a loop to retrieve additional pages of results
			...
			theURL <- httr::GET("https://api.elsevier.com/content/search/scopus", query = list(apiKey = key, query = string,  sort = mySort, httpAccept = "application/xml", view = "standard", count = retCount, cursor = cursor)) ## get the next page of results
...
searchByID <- function(theIDs, idtype, datatype = "application/xml", content = "complete", myStart = 0, retCount = 25, outfile) {
	##library(httr)
	##library(XML)
	key <- "ea25caf52b9b1fcd980c3f26ac8f9a77"
	if (length(theIDs) == 1) {
